...
stimEdit.locCurrElem = [];
stimEdit.locCurrElem = RunClass();
stimEdit.locCurrElem.Copy(maingui.dataTree.currElem);
...
conditions =  stimEdit.locCurrElem.GetConditions();
...
if isempty(stimEdit.locCurrElem)
    return;
end
if ~isa(stimEdit.locCurrElem, 'RunClass')
    return;
end

% Now that we made sure legit dataTree exists, we can match up
% the selected stims to the stims in currElem
t = stimEdit.locCurrElem.GetTimeCombined();
s = stimEdit.locCurrElem.GetStims(t);
...
global stimEdit
t = stimEdit.locCurrElem.GetTime();
if ~all(t1==t2)
...
tc             = stimEdit.locCurrElem.GetTime();
...
if isempty(iS_lst)
    ...
    stimEdit.locCurrElem.AddStims(tc(tPts_idxs_select), CondName);
    stimEdit.status = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if menu_choice==nActions-1 & nActions==nCond+4
        ...
        stimEdit.locCurrElem.DeleteStims(tc(tPts_idxs_select));

    %%%% Toggle active/inactive stim
    elseif menu_choice==nActions-2 & nActions==nCond+4

        stimEdit.locCurrElem.ToggleStims(tc(tPts_idxs_select));
    ...
    elseif menu_choice<=nCond+1
        ...
        stimEdit.locCurrElem.MoveStims(tc(tPts_idxs_select), CondName);
...
stimEdit.locCurrElem.SetStimDuration(icond, duration);
...
duration = stimEdit.locCurrElem.GetStimDuration(icond);


% -------------------------------------------------------------------
function [tpts, duration, vals] = GetStimData(icond)
global stimEdit
[tpts, duration, vals] = stimEdit.locCurrElem.GetStimData(icond);


% -------------------------------------------------------------------
function SetStimData(icond, data)
global stimEdit
stimEdit.locCurrElem.SetStimTpts(icond, data(:,1));
stimEdit.locCurrElem.SetStimDuration(icond, data(:,2));
stimEdit.locCurrElem.SetStimValues(icond, data(:,3));
...
if ~stimEdit.locCurrElem.AcquiredDataModified()
    return
end
...
if ~strcmpi(stimEdit.config.autoSaveAcqFiles, 'Yes')
    ...
    if stimEdit.locCurrElem.IsRun()
        msg = sprintf('Are you sure you want to save stimulus edits directly in the acquisition file %s?', stimEdit.locCurrElem.name);
    else
        msg = sprintf('Are you sure you want to save stimulus edits directly in acquisition files in %s?', stimEdit.locCurrElem.name);
    end
    ...
    stimEdit.dataTree.currElem.Copy(stimEdit.locCurrElem);
else
    % Otherwise auto-save 
...
h = waitbar_improved(0, 'Saving new stim marks to %s...', stimEdit.locCurrElem.name);
...
waitbar_improved(1, h, 'Saving new stim marks to %s...', stimEdit.locCurrElem.name);
...
conditions = stimEdit.locCurrElem.GetConditions();
filename = stimEdit.locCurrElem.GetName();
...
if stimEdit.locCurrElem.iRun==0
    return;
end

% Load current element data from file
if stimEdit.locCurrElem.IsEmpty()
    stimEdit.locCurrElem.Load();
end
...
set(handles.popupmenuConditions, 'string', sort(stimEdit.locCurrElem.GetConditions()));
conditions = get(handles.popupmenuConditions, 'string');
idx = get(handles.popupmenuConditions, 'value');
condition = conditions{idx};
SetUitableStimInfo(condition, handles);
...
for i = 1:length(onsets)
    stimEdit.locCurrElem.AddStims(onsets(i), cond);
end
iG = stimEdit.dataTree.GetCurrElemIndexID();
stimEdit.dataTree.groups(iG).SetConditions();
if ~isempty(stimEdit.updateParentGui)
...
try
    ...
    global stimEdit
    aux = stimEdit.locCurrElem.acquired.aux;
